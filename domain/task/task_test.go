...
func TestValidate(t *testing.T) {
	testCases := []struct {
			...
			{
					name:        "Valid task",
					task:        task.NewTask("task1", user.UserId(1), "2024-01-01", 1),
					expectedErr: "",
			},
			{
					name:        "Invalid task name",
					task:        task.NewTask("", user.UserId(1), "2024-01-01", 1),
					expectedErr: "invalid task name",
			},
			{
					name:        "Invalid user ID",
					task:        task.NewTask("task1", user.UserId(0), "2024-01-01", 1),
					expectedErr: "invalid user id",
			},
			{
					name:        "Invalid due date",
					task:        task.NewTask("task1", user.UserId(1), "", 1),
					expectedErr: "invalid due date",
			},
			{
					name:        "Invalid delay count",
					task:        task.NewTask("task1", user.UserId(1), "2024-01-01", -1),
					expectedErr: "invalid delay count",
			},
	}
